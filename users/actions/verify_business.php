...
// Instantiate the verification oobject
$verifiedBusiness = new Verify();
if(isset($_POST['verify_my_business']))
{
  ...
  $user_id = Database::checkInput($_POST['user_id']);
  ...
  $local_government = ((isset($_POST['local_government']))? Database::checkInput($_POST['local_government']) : '' );
  ...
  {
    $error = "All fields with * are rrequired ";
  }
  ...
  if($error)
  {
    ...
    header("Location: ../../profiles/verify_business.php?error_message=". urlencode($error));
  }
  else
  { 
    ...
    $message = "Verification information sent successfully";
    $verifiedBusiness->verifyBusiness($business_name, $email, $business_phone,$registration_number,$local_government,$states,$description,$business_address,$user_id);
